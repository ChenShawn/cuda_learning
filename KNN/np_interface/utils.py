...
def gen_laplacians(data, k=30):
    ...
    c_k = ct.c_int(k)
    dll.gen_laplacian(c_data, c_num_point, c_num_graphs, c_k, c_laplacian)
    return laplacian
...
def random_walk_pooling(input_arr, adjacency, K=50):
    if adjacency is None:
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        dll = np.ctypeslib.load_library(os.path.join(BASE_DIR, 'graph_op.so'), '.')
        adjacency = np.mat(dll.gen_graph(input_op, k=K))
    D = np.mat(np.diag(np.sum(adjacency, axis=1)), dtype=np.float32)
    laplacian = D.I * adjacency
    rank = np.random.uniform(0.0, 1.0, [input_op.shape[0]])
    rank /= np.sum(rank)
    rank = np.mat(rank).T
    for it in range(100):
        rank = laplacian * rank
    return np.argsort(rank, axis=0)
